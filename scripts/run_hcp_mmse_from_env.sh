...
# Keep baseline outputs isolated per config file instead of inheriting a shared env path.
unset HCP_MMSE_OUTPUT_DIR
unset HCP_MMSE_CACHE_DIR
...
if [ "$RUN_STATUS" != "success" ]; then
  echo "HCP baseline run failed. See: $RUN_LOG" >&2
  exit 1
fi
