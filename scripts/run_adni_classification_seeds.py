...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ADNI classification config across multiple seeds.")
    parser.add_argument("--config", type=Path, required=True, help="Base experiment config YAML.")
    parser.add_argument("--seeds", type=int, nargs="+", required=True, help="Seed values to run.")
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("outputs/adni_seed_runs"),
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        command = [
            sys.executable,
            "-m",
            "brainage.experiments.run_adni_classification",
            "--config",
            str(config_path),
        ]
