...
ADNI_CONFIGS_RAW="${4:-$PROJECT_ROOT/configs/experiment/adni_mmse_transfer_full_ft.yaml}"
RUN_ADNI_ANALYSIS="${RUN_ADNI_ANALYSIS:-1}"
...
IFS=',' read -r -a ADNI_CONFIGS <<< "$ADNI_CONFIGS_RAW"
for raw_config in "${ADNI_CONFIGS[@]}"; do
  config="$(echo "$raw_config" | xargs)"
  if [ -z "$config" ]; then
    continue
  fi
  echo "==> Run ADNI MMSE transfer: $config"
  bash "$PROJECT_ROOT/scripts/run_adni_mmse_from_env.sh" "$ENV_FILE" "$config"
done

if [ "$RUN_ADNI_ANALYSIS" = "1" ]; then
  echo "==> Run ADNI diagnosis-aware analysis"
  bash "$PROJECT_ROOT/scripts/run_adni_diagnosis_analysis_from_env.sh"
fi

echo "==> Completed HCP, OASIS, and ADNI regression reruns"
