...
CONFIG_FILE="${2:-$PROJECT_ROOT/configs/experiment/oasis_mmse_transfer_full_ft.yaml}"
...
# Keep transfer outputs isolated per config file instead of inheriting a shared env path.
unset OASIS_MMSE_OUTPUT_DIR
unset OASIS_MMSE_CACHE_DIR
unset SOURCE_CHECKPOINT_PATH
...
printf '%s\n' "conda run -n $CONDA_ENV_NAME python -m brainage.experiments.run_oasis_transfer --config $CONFIG_FILE_ABS" > "$RUN_RECORD_DIR/command.txt"

RUN_STATUS=success
export BRAINAGE_RUN_RECORD_DIR="$RUN_RECORD_DIR"
export BRAINAGE_RUN_STARTED_AT="$STARTED_AT"
cd "$PROJECT_ROOT"
if ! env BRAINAGE_RUN_STATUS=success conda run -n "$CONDA_ENV_NAME" python -m brainage.experiments.run_oasis_transfer --config "$CONFIG_FILE_ABS" 2>&1 | tee "$RUN_LOG"; then
  RUN_STATUS=failed
fi
...
if [ "$RUN_STATUS" != "success" ]; then
  echo "OASIS transfer run failed. See: $RUN_LOG" >&2
  exit 1
fi

echo "OASIS transfer run completed successfully."
