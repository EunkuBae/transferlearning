...
CONFIG_FILE="${2:-$PROJECT_ROOT/configs/experiment/adni_mmse_transfer_full_ft.yaml}"
...
readarray -t RUN_METADATA < <(
  PROJECT_ROOT="$PROJECT_ROOT" CONFIG_FILE="$CONFIG_FILE" "$PYTHON_BIN" - <<'PY'
import os
...
printf '%s\n' "conda run -n $CONDA_ENV_NAME python -m brainage.experiments.run_adni_mmse_transfer --config $CONFIG_FILE_ABS" > "$RUN_RECORD_DIR/command.txt"

RUN_STATUS=success
export BRAINAGE_RUN_RECORD_DIR="$RUN_RECORD_DIR"
export BRAINAGE_RUN_STARTED_AT="$STARTED_AT"
cd "$PROJECT_ROOT"
if ! env BRAINAGE_RUN_STATUS=success conda run -n "$CONDA_ENV_NAME" python -m brainage.experiments.run_adni_mmse_transfer --config "$CONFIG_FILE_ABS" 2>&1 | tee "$RUN_LOG"; then
  RUN_STATUS=failed
fi
...
STARTED_AT="$STARTED_AT" ENDED_AT="$ENDED_AT" "$PYTHON_BIN" - <<'PY'
import json
import os
from pathlib import Path
...
if [ "$RUN_STATUS" != "success" ]; then
  echo "ADNI MMSE transfer run failed. See: $RUN_LOG" >&2
  exit 1
fi

echo "ADNI MMSE transfer run completed successfully."
