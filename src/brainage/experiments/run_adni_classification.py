"""Run ADNI baseline T1 classification with a 3D CNN."""
...
try:
    ...
    raise RuntimeError(
        "Running ADNI classification experiments requires `PyYAML`. Install dependencies with `pip install -r requirements.txt`."
    ) from exc


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ADNI baseline 3D-CNN classification training.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiment/adni_cls_baseline_linux.yaml"),
        help="Path to ADNI classification experiment config YAML.",
    )
...
def main() -> None:
    ...
    output_dir = resolve_config_path(
        raw_value=str(output_config.get("run_dir", "outputs/adni_cls_baseline")),
    ...
    if output_config.get("cache_dir") is not None or output_config.get("cache_dir_env") is not None:
        cache_dir = resolve_config_path(
            raw_value=str(output_config.get("cache_dir", "outputs/cache/adni_cls_baseline")),
    ...
    metrics_payload = {
        "experiment_name": config.get("experiment_name", "adni_cls_baseline"),
        "use_demographics": use_demographics,
